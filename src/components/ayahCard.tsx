...
export default function AyahCard() {
  ...
  return (
    <div className="w-[55%] sm:w-[80%] flex flex-col items-center">
      {/* top section of details surah pages */}
      <div className="flex w-full items-center justify-between mb-6">
        <a
          href="/"
          className="inline-flex items-center gap-2 text-sm text-[#0D6549] hover:text-[#0d65498a]"
        >
          <svg
        ...
        <div className="flex gap-2">
          <a
            ...
            className="inline-flex items-center gap-1 rounded-lg border border-border px-3 py-1.5 text-sm hover:bg-white"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="24"
      ...
      <div className="text-center w-full bg-white mb-8 rounded-2xl border-[0.5px] border-[#0d654947] p-6">
        <p className="text-sm text-muted-foreground uppercase tracking-wider">
          Surah {qurans[Number(params.slug) - 1].id}
        </p>
        <h1 className="text-3xl font-bold mt-1">
          {qurans[Number(params.slug) - 1].transliteration}
        </h1>
        <p className="text-muted-foreground italic">
          {qurans[Number(params.slug) - 1].translation}
        </p>
        <p className="mt-3 text-[26px] text-[#0D6549] font-amiri">
          {qurans[Number(params.slug) - 1].name}
        </p>

        <p className="text-xs text-muted-foreground mt-2">
          {qurans[Number(params.slug) - 1].type} ·{" "}
          {qurans[Number(params.slug) - 1].total_verses} ayahs
        </p>
      </div>
      ...
      <div className="w-full flex flex-col gap-5">
        {surah.verses.map((ayah, index) => (
          <div className="rounded-xl p-5 border-[0.5px] bg-white border-[#0d654947]">
            ...
            <p className="mt-4 text-[13px] text-[#262c2ab0] leading-relaxed">
              {ayah.translation}
            </p>
          </div>
        ))}
